...
First of all we need to prevent Node.js process exiting. To do this we can use the method `process.stdin.resume` that cause the process to pause and override disable default behavior like exiting on `Ctrl+C`.
...
Thanks to `process.stdin.resume` our server's process doesn't exit after sending the request. We need to handle update the code that respond with the challenge to kill our server's process.
