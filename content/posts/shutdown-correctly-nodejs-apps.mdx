...
They are two changes. When server start listening on port 9090 we send a first request to subscribe ou server to the webhook.
...
We also changed our server's request handle to let him confirm the subscription to the webhook by responding the challenge.
...
Let's see how our server acts when we try to shutdown it. I added some logs to make it more visual.

![Error when killing server with webhook](/images/kill-webhook.gif)

As you can see, it doesn't shutdown properly. The server's process is killed before we get the webhook request that confirm unsubscription so the webhook keep sending events to our server.

To fix this we need to prevent Node.js process exiting. We can use the method `process.stdin.resume` that cause the process to pause and override disable default behavior like exiting on `Ctrl+C`.
...
Nice, it now wait the confirmation before exiting the process.
